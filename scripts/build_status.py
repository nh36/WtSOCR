...
class ReleaseStats:
    ...
    manifest_recipe_revision: str
    manifest_production_input_origin: str
...
def parse_manifest() -> tuple[str, str, str, list[str]]:
    manifest = RELEASE / "manifest.md"
    text = manifest.read_text(encoding="utf-8") if manifest.exists() else ""
    generated = "unknown"
    recipe_revision = "unknown"
    production_input_origin = "unknown"
    manifest_volumes: list[str] = []
    m = re.search(r"Generated UTC:\s*`([^`]+)`", text)
    if m:
        generated = m.group(1)
    m = re.search(r"Build recipe revision:\s*`([^`]+)`", text)
    if m:
        recipe_revision = m.group(1)
    else:
        m = re.search(
            r"Source/code commit observed while building this bundle:\s*`([^`]+)`",
            text,
        )
        if m:
            recipe_revision = m.group(1)
    m = re.search(r"Production input provenance:\s*`([^`]+)`", text)
    if m:
        production_input_origin = m.group(1)
    ...
    return generated, recipe_revision, production_input_origin, manifest_volumes
...
def read_release_stats() -> ReleaseStats:
    generated, recipe_revision, production_input_origin, manifest_volumes = (
        parse_manifest()
    )
    volumes: dict[str, dict[str, int | str]] = {}
    ...
    return ReleaseStats(
        ...
        manifest_recipe_revision=recipe_revision,
        manifest_production_input_origin=production_input_origin,
...
def build_family_rows(stats: ReleaseStats) -> list[dict[str, str]]:
    ...
    rows = [
        ...
        row(
            ...
            "Reviewed reference-marker rows are exact page-line-token corrections; direction is determined from lemma-order comparison where possible; ambiguous lemma identity is accepted only when all possible matches point the same direction; slash is not bulk-promoted; remaining rows are diagnostic only; superscripted marker numbers are not inferred automatically.",
        ),
        row(
...
def remaining_work_rows(stats: ReleaseStats) -> list[list[str | int]]:
    ...
    return [
        ...
        [
            ...
            "Promote only reviewed page/line/token rows with direction determined from lemma order where possible; ambiguous lemma identity is acceptable only when all possible matches point the same direction; do not apply broad T/I/slash/backslash -> ↑/↓, bulk-promote slash, or infer superscript markers automatically.",
...
- Build recipe revision: `{stats.manifest_recipe_revision}`
- Production input provenance: `{stats.manifest_production_input_origin}`
- Volumes included: `{', '.join(VOLUME_ORDER)}`
- Policy: base OCR authoritative; Google Vision alternate witness only.

The build recipe revision identifies the pinned code used for deterministic
reproduction. Production-input provenance separately records the historical
workspace and revision that produced the locked payload. The runtime checkout
is reported by the reproduction command and is not inferred from either field.
